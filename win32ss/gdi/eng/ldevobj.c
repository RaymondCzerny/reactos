    ...
    _Out_ PDEVMODEW *ppdm)
{
    ...
    TRACE("ldevGetDriverModes('%ls', %p)\n", pwszDriverName, hDriver);

    pldev = ldevLoadDriver(pwszDriverName, LDEV_DEVICE_DISPLAY);
    if (!pldev)
        goto cleanup;
...
leave:
    ...
    TRACE("ldevLoadDriver returning %p\n", pldev);
    return pldev;
}
    ...
    _Inout_ PGRAPHICS_DEVICE pGraphicsDevice)
{
    ...
    for (; *pwsz; pwsz += wcslen(pwsz) + 1)
    {
        ...
        cbSize = ldevGetDriverModes(pwsz, pGraphicsDevice->DeviceObject, &pdm);
        if (!cbSize)
        {
...
EngLoadImage(
    _In_ LPWSTR pwszDriverName)
{
    return (HANDLE)ldevLoadDriver(pwszDriverName, LDEV_IMAGE);
}
