    ...
    PPDEVOBJ ppdev)
{
    ...
    if (DrvProbeAndCaptureDevmode(pGraphicsDevice, ppdev->pdmwDev, &newDevMode))
    {
        ExFreePoolWithTag(ppdev->pdmwDev, GDITAG_DEVMODE);
    ...
    PDEVMODEW pdm)
{
    ...
    ppdev->pldev = ldevLoadDriver(pdm->dmDeviceName, LDEV_DEVICE_DISPLAY);
    if (!ppdev->pldev)
    {
    ...
    if (!DrvProbeAndCaptureDevmode(pGraphicsDevice, pdm, &newDevMode))
    {
        DPRINT1("DrvProbeAndCaptureDevmode() failed\n");
        PDEVOBJ_vRelease(ppdev);
        return NULL;
    }
    ppdev->pdmwDev = newDevMode;
    ...
    PDEVMODEW pdm)
{
    ...
    // pdm = DrvProbeAndCaptureDevmode(ppdev, pdm);
